    ...
    def _build_graph(self):
        ...
        def chatbot(state: State):
            ...
            try:
                ...
                experimental_msg = "你在收到请求后，首先必须思考是否要从记忆工具中检索记忆，你在给出最终回复之前，必须思考是否需要把有用的信息存入记忆工具。"
                ...
                style_msg = '默认使用简体中文，如非特殊要求，禁止使用markdown语法特别是"**"。你处在一个群聊之中，因此你的回复像人类一样使用口语化的连续文字，不会轻易使用列表分点。你的回复往往20-50字，最长不超过100字。但是基于搜索结果回答时，你可以突破字数限制适当增加字数，确保信息完整。你回答的长度应该像人类一样灵活，避免每次回复都是相同的长度。对于评价、偏好、选择，你必须做出选择不能骑墙。图片链接必须换行在新的一行以[IMAGE]url[/IMAGE]的格式输出，每个一行，禁止使用其它格式。'
