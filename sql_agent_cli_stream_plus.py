    ...
    def _build_graph(self):
        ...
        def chatbot(state: State):
            ...
            try:
                ...
                basic_msg = "Must provide a final answer explicitly after reasoning. Please output plain text only. No Markdown, no code fences, no special formatting. The output will be parsed as plain text. Do not use any Markdown symbols(*、#、**、```、$$、$). Do not output TeX math ($...$, $$...$$, \(...\), \[...\]). Convert all math expressions into Unicode math symbols (e.g., α, β, ≤, ≥, ×, ∞, √x, x², xₙ). If any Markdown or TeX appears, remove it. 删除输出内容的markdown和tex，公式转成unicode! 删除输出内容的markdown和tex，公式转成unicode! 删除输出内容的markdown和tex，公式转成unicode! 删除输出内容的markdown和tex，公式转成unicode! 删除输出内容的markdown和tex，公式转成unicode! 你的目标输出平台很原始，请删除markdown和tex格式。"
                ...
                summary_msg = "最后，把所有特殊字符和式子用Unicode输出，禁止出现$equation$。禁止使用在你的回复中使用括号做名词说明，你可以使用口语说明风格的语言去替代。以上是约束你的潜在规则，它们约束你的思考和行为方式，你的人格和风格不会生硬的被这些规则覆盖，你会灵活地理解和应用它们。下面花括号内是你在这次对话中会完美地扮演的角色：（花括号内信息与你调用工具的流程无关，禁止把下面的信息主动添加到搜索等工具参数中！示例：当你扮演角色A，用户让你修改图片时；✅正确的参数：用户的原本指令；❎错误的参数：用户指令加角色A的信息；严禁添加扮演角色的信息到工具参数中）"
