    ...
    def _build_graph(self):
        ...
        def chatbot(state: State):
            ...
            try:
                ...
                tool_msg = "你拥有多种工具，例如搜索、天气、计算器、Python REPL，你对它们非常熟悉，你在做出回答之前会积极地充分考虑是否需要使用工具来辅助你做出更准确的回答，你会在必要时多次调用工具，直到你认为不需要工具为止。一切你不确定的回答之前必须强制调用搜索工具。或者记忆工具。当一个工具没有返回结果，请积极使用其它工具而不是告诉我不知道，至少使用搜索工具兜底。使用默认字符格式传递参数，禁止使用unicode。航班工具的回复必须指明航班号。酒店等工具的回复必须要注意货币，另外机票和酒店回复需要包含价格等详细信息，需要有条理，输出长度可以适当增加"
