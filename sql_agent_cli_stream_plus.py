...
class SQLCheckpointAgentStreamingPlus:
    ...
    def _restore_timers_from_store(self) -> None:
        ...
        def _schedule_one(rec: dict) -> None:
            ...
            t.start()
            print(
                f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m[TimerStore] 恢复计时器：{remain} 秒后将在群 {group_id} 内提醒 @({user_id})：{desc}",
                flush=True,
            )
    ...
    def _load_sys_msg_content(self) -> str:
        ...
        with open(abs_path, "r", encoding="utf-8") as f:
            content = f.read()
            print(f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m[SysInfo] 已加载文件: {abs_path}，长度 {len(content)} 字符。")
            # 打印头尾各50字符,仅输出文本不要格式符号
            print(
                f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m[SysInfo] Prompt内容预览: {content[:50].replace(chr(10), ' ')} ... {content[-50:].replace(chr(10), ' ')}"
            )
            ...
            if self._enable_tools:
                ...
                if os.environ.get("OPENWEATHERMAP_API_KEY"):
                    ...
                    def get_weather(location_en_name: str) -> str:
                        ...
                        print(f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m[Weather Tool Output] {result}", flush=True)  # 调用时直接打印
                        return result
                ...
                def set_timer(
                    ...
                    def _send_group_at_message_later() -> None:
                        ...
                        try:
                            ...
                            print(
                                f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m\033[33m[TimerTool]\033[0m 计时器触发，已在群 {group_id} 内提醒 @({user_id})：{description}",
                                flush=True,
                            )
                        except Exception as e:
                            # 打印到标准错误便于排查，不吞异常
                    ...
                    t.start()
                    print(
                        f"\n\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m\033[33m[TimerTool]\033[0m 已创建计时器：{seconds} 秒后将在群 {group_id} 内提醒 @({user_id})：{description}",
                        flush=True,
                    )
                ...
                try:
                    ...
                    # 未安装或失败时跳过，不影响其它工具
                    print(
                        f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m[Warn] langmem 工具加载失败，跳过。错误信息：{e}", flush=True
                    )
                    pass
                ...
                def hotel_search(city: str, limit: int = 5) -> str:
                    ...
                    print(f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m[Hotel Tool] 查询结果：\n" + "\n".join(lines), flush=True)
                    return "\n".join(lines)
    ...
    def set_token_printer(self, fn: Callable[[str], None]) -> None:
        def _wrapped(s: str) -> None:
            ...
            if not getattr(self, "_agent_header_printed", False):
                print(f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m\033[32m[Reply]\033[0m Agent: ", end="", flush=True)
        ...
        try:
            for ev in self._graph.stream(
                ...
                if label == "Tool":  # and not tool_notified:
                    ...
                    print(f"\033[94m{time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime())}\033[0m\033[33m[Tool]\033[0m Calling tool [{name}]")
                    # tool_notified = True
                if label == "Agent":
