...
def test_ensure_gemini_env_once_accepts_vertex_service_account_json(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """
    验证 Vertex AI 服务账号 JSON 配置可通过校验。
    ...
    Raises:
        None: 测试用例不主动抛出异常。
    """
    credentials_path = tmp_path / "vertex-sa.json"
...
def test_ensure_gemini_env_once_rejects_missing_credentials() -> None:
    """
    验证缺少 AI Studio 与 Vertex 凭证时会抛出断言错误。
    ...
    Raises:
        None: 测试用例不主动抛出异常。
    """
    with pytest.raises(AssertionError, match="缺少 Gemini 可用凭证"):
        target._ensure_gemini_env_once()
