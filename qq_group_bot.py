...
def main() -> None:
    ...
    daily_question = "这是一个每日简报任务,请不要啰嗦，你需要包含：1.今天的日期与星期，现在是几点几分，今天是否是节日或者特殊的日子（中国/日本/世界通用）[没有就不回答，不要声明不是节日，跳过这一段]，最近一周会不会有法定假日（中国/日本）[没有就不回答，不要声明不是法定假日，跳过这一段][分段]；2.今天京都的天气怎么样（配上对应的emoji，必须有气温，如果下雨需要精确到小时）[分段]；3.使用抽选工具list模式，参看是否有今日截止的抽选，如果有就告知[没有就不回答，跳过这一段]。4.以及想说的话，一句话即可。\n总字数控制在大于180字，280字以内。"
    ...
    nightly_question = "这是一个晚间每日总结任务，请不要啰嗦，你需要包含：1.现在是几点几分，明天是星期几，明天是否是重要节假日（中国/日本/世界通用）[没有就不回答，不要声明不是节日，不要提示非常不重要的节日][分段]；2.明天京都的天气预报如何（配上对应的emoji，必须有气温，如果下雨需要精确到小时）[分段]；3.以及想说的话，一句话即可。\n总字数控制在大于180字，250字以内。"
