...
def _build_daily_question(daily_city: str) -> str:
    ...
    return (
        ...
        "世界纪念日。\n"
        f"2. 使用visual_crossing_weather查询今天{city}的逐小时天气，hour设为true。天气部分按以下格式输出，每项单独一行：{{天气emoji}} {city}｜{{主要天气}} {{最低温}}～{{最高温}}℃；🌡️体感温度：{{最低体感温度}}～{{最高体感温度}}℃；🌧️降水时段：{{降水时段}}；⚠️气象预警：{{预警内容}}。没有降水或预警时省略对应行；湿度、风速等只在明显异常时按{{对应emoji}}{{名称}}：{{内容}}另起一行。\n"
