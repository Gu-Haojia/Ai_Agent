...
def _collect_image_urls(item: dict[str, object], limit: int = 3) -> tuple[str, ...]:
    ...
    Args:
        item (dict[str, object]): Mercari 返回的商品字段。
        limit (int): 返回的最大图片数量。

    Returns:
        tuple[str, ...]: 去重后的图片 URL（最多 limit 条）。
...
class MeruSearchResult:
    ...
    Attributes:
        ...
        url (str): 商品链接。
        image_urls (tuple[str, ...]): 商品图片 URL 列表（最多若干缩略图）。
    """
...
class _MeruWatchTask:
    ...
    def _dispatch(
        ...
        Args:
            message (str): 需要发送的文本。
            items (Sequence[MeruSearchResult]): 对应的商品列表。
            tag (str): 当前通知标签。
            price_channel (bool): True 表示走价格提醒渠道。

        Returns:
            None: 无返回值。

        Raises:
            AssertionError: 当 message 为空时抛出。
        """
        assert message.strip(), "通知消息不能为空"
        if price_channel:
