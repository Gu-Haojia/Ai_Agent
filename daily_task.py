...
class DailyWeatherTask:
    ...
    def __init__(
        ...
        *,
        ...
        agent: Optional[SQLCheckpointAgentStreamingPlus] = None,
    ) -> None:
        """
        初始化调度器。
        ...
        Args:
            ...
            agent_provider (Optional[AgentProvider]): 获取 Agent 的回调，不提供时会回退到 agent。
            agent (Optional[SQLCheckpointAgentStreamingPlus]): 备用的初始 Agent 实例，用于未提供回调时。
        ...
        assert agent_provider or agent, "必须提供 agent_provider 或 agent 其中之一"
        ...
        self._agent_provider: AgentProvider = agent_provider or (lambda: agent)  # type: ignore[arg-type]
