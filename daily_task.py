...
def build_daily_question() -> str:
    ...
    return (
        ...
        f"2. 使用合适的天气工具查询今天{city}的逐小时天气，hour设为true。天气部分按以下格式输出，每项单独一行：今日{{目标地点}}的天气：；{{天气emoji}} {{主要天气}} {{最低温}}～{{最高温}}℃；🌡️体感温度：{{最低体感温度}}～{{最高体感温度}}℃；☔️降水时段：{{降水时段}}；⚠️气象预警：{{预警内容}}。没有降水或预警时省略对应行；湿度、风速或降水量明显异常时，分别按💧相对湿度：{{相对湿度}}、💨风速：{{风速}}、💦降水量：{{降水量}}另起一行。\n"
...
def build_nightly_question() -> str:
    ...
    return (
        ...
        f"2. 使用合适的天气工具查询明天{city}的逐小时天气，hour设为true。天气部分按以下格式输出，每项单独一行：明日{{目标地点}}的天气：；{{天气emoji}} {{主要天气}} {{最低温}}～{{最高温}}℃；🌡️体感温度：{{最低体感温度}}～{{最高体感温度}}℃；☔️降水时段：{{降水时段}}；⚠️气象预警：{{预警内容}}。没有降水或预警时省略对应行；湿度、风速或降水量明显异常时，分别按💧相对湿度：{{相对湿度}}、💨风速：{{风速}}、💦降水量：{{降水量}}另起一行。\n"
