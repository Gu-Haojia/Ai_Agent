...
def build_daily_question() -> str:
    ...
    return (
        ...
        "1. 用一段自然的话介绍今天的日期、星期和东京时间。如果今天是中国或日本的传统节日或法定假日，自然地带出节日信息。查询接下来7天内中国和日本的法定假日或传统节日；如有，用自然的方式提醒具体日期和节日，不要在正文中说明查询范围。没有相关节日时直接略过，不要输出否定说明。不要输出国际日或世界纪念日。\n"
        f"2. 使用合适的天气工具查询今天{city}的逐小时天气，hour设为true。天气部分按以下格式输出，每项单独一行：今日{{目标地点}}的天气：；{{天气emoji}} {{主要天气}} {{最低温}}～{{最高温}}℃；🌡️体感温度：{{最低体感温度}}～{{最高体感温度}}℃；☔️降水时段：{{降水时段}}；🌅日出日落：{{日出日落时间}}；⚠️气象预警：{{预警内容}}。没有降水或预警时省略对应行；湿度、风速、降水量或紫外线指数明显异常时，分别按💧相对湿度：{{相对湿度}}、💨风速：{{风速}}、💦降水量：{{降水量}}、🕶️紫外线：{{紫外线指数}}另起一行。\n"
        "3. 调用imas_ticket_tool的list模式；只有存在今天截止的抽选时才输出，告知今日截止的抽选，名称保持原文，相关信息交代清楚。\n"
        "4. 最后，适当联系上文，自由说一段想说的话，80字以内\n\n"
...
def build_nightly_question() -> str:
    ...
    return (
        ...
        "1. 用一段自然的话介绍现在的东京时间、明天的日期和星期。如果明天是中国或日本的传统节日或法定假日，自然地带出节日信息。查询从明天起7天内中国和日本的法定假日或传统节日；如有，用自然的方式提醒具体日期和节日，不要在正文中说明查询范围。没有相关节日时直接略过，不要输出否定说明。不要输出国际日或世界纪念日。\n"
        f"2. 使用合适的天气工具查询明天{city}的逐小时天气，hour设为true。天气部分按以下格式输出，每项单独一行：明日{{目标地点}}的天气：；{{天气emoji}} {{主要天气}} {{最低温}}～{{最高温}}℃；🌡️体感温度：{{最低体感温度}}～{{最高体感温度}}℃；☔️降水时段：{{降水时段}}；🌅日出日落：{{日出日落时间}}；⚠️气象预警：{{预警内容}}。没有降水或预警时省略对应行；湿度、风速、降水量或紫外线指数明显异常时，分别按💧相对湿度：{{相对湿度}}、💨风速：{{风速}}、💦降水量：{{降水量}}、🕶️紫外线：{{紫外线指数}}另起一行。\n"
        "3. 最后，适当联系上文，自由说一段想说的话，80字以内\n\n"
        "各部分之间空行，不要为了凑字数补充内容。正文第一行开头不要添加emoji。"
    )
...
class DailyTicketTask:
    ...
    def __init__(
        ...
        prompt: str = "检测到偶像大师抽选更新了，请使用工具的update模式，整理详细的新抽选信息列表。并创建所有新增抽选截止时间提前一小时的reminder（提醒必须有明确是针对什么公演的什么抽选），通知群1070489110的用户2920504178，不要在回复中包含具体群号和账号。",
